...
import { fetchCalendarAnniversariesExternal } from './anniversaries.source'
...
async function handler(req: Request) {
  ...
  const external = await fetchCalendarAnniversariesExternal({
    year,
    month,
    revalidateSec: 60 * 60 * 12,
  })
...
export const GET = handleApi(handler, {
  tag: '[api.calendar.anniversaries]',
  internalMessage: 'calendar anniversaries fetch failed',
})
