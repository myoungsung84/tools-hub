...
import { fetchCalendarSundryExternal } from './sundry.source'
...
async function handler(req: Request) {
  ...
  const external = await fetchCalendarSundryExternal({ year, month, revalidateSec: 60 * 60 * 12 })
...
export const GET = handleApi(handler, {
  tag: '[api.calendar.sundry]',
  internalMessage: 'calendar sundry fetch failed',
})
